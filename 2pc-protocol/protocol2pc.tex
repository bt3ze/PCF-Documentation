...
\begin{enumerate}
	...
	\todo[inline]{How much randomness does $Gen$ need, or equivalently, how long should the seed $\rho$ be? There must be some relationship to the security parameter $k$.}
	...
	Here, we set the length of $\rho$ to be $k \times m_{1}$, and $\rho$ completely determines the two-universal circuit $H$ that computes $h$ by matrix multiplication.
	...
	\begin{enumerate}[label=(\alph*)]
		...
		\todo[inline]{Does $\Phi$ not require randomness in its commitments? Is the output of a hash function like SHA enough?}
