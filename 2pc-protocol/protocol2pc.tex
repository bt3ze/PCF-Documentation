...
\todo[inline]{In the code, this protocol randomly selects $Y_{0}$ and $Y_{1}$ before computing $Y_{b} = Y_{b} \cdot (gr_{b})^{s_{b}} \cdot (hr_{b})^{t_{b}}$ and does not include any of Sender's inputs. The comments indicate that $Y_{0}$ and $Y_{1}$ are chosen uniformly at random, as the keys are, but the code may be in need of this simple change. }
